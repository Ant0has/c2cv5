...
const Calculator: FC<IProps> = () => {
  ...
  const [planCoefficient, setPlanCoefficient] = useState(
    storagedData ? JSON.parse(storagedData) : prices
  );
  ...
  const handleChangePlanCoefficient =
    (key: string) => (e: ChangeEvent<HTMLInputElement>) => {
      const val = e.target.value;

      const changedData = { ...planCoefficient, [key]: val };

      setPlanCoefficient(changedData);

      console.log("changedData", changedData);

      localStorage.setItem("PLAN_COEFFICINT", JSON.stringify(changedData));
    };
  ...
  }, [arrivalPoint, departurePoint, planCoefficient, map]);

  useEffect(() => {
    handleCalculate();
  }, [handleCalculate, planCoefficient]);
        ...
        {plans.map((plan) => (
          <div key={plan.key} className={clsx(s.container)}>
            ...
            <input
              ...
              onChange={handleChangePlanCoefficient(plan.key)}
            />
          </div>
        ))}
        <div
          ...
          onClick={() => {
            ...
            localStorage.setItem(PLAN_COEFFICINT, JSON.stringify(prices));
          }}
        >
          Вернуть значения по умолчаню
        </div>
      </div>
