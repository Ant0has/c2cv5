...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const canonicalUrl = `${BASE_URL}/morskoj-otdyh/${params.destination}`;
  ...
  const descriptionParts = [
    `Заказать трансфер ${routeName} к морю.`,
  ...
  const keywords = destination.seoKeywords ||
    `трансфер ${destination.fromCity}, такси ${routeName}, трансфер на море ${destination.toCity}, заказать трансфер`;
